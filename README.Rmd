...
### Install
`distreg.vis` is not on CRAN, but only on GitHub at the moment. Run the following code to install the package and all its extensions. The code also installs a new version of `bamlss`, which is not on CRAN yet but includes necessary functions.
```{r, eval = FALSE}
if (!require(devtools))
